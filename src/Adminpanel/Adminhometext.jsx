...
import { useCreateTexts, useDeleteTexts, useGetAllTextss, useUpdateTexts } from '../api/textapiHome/textapihome';

const Adminhometext = () => {
  const { data: texts, isError, isLoading } = useGetAllTextss();
  const createText = useCreateTexts();
  const updateTextMutation = useUpdateTexts();
  const deleteText = useDeleteTexts();
  ...
  return (
    <div className="min-h-screen p-8 bg-gray-50">
          ...
          <div key={text.id} className="p-6 bg-white border border-gray-300 rounded-lg shadow-lg">
            ...
            <div className="flex space-x-2">
              <button
                ...
                className="px-4 py-2 text-sm font-semibold text-black bg-blue-500 rounded-md shadow-md hover:bg-blue-600 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2"
              >
                Edit
              </button>
              <button
                onClick={() => handleDelete(text.id)}
                className="px-4 py-2 text-sm font-semibold text-black bg-red-500 rounded-md shadow-md hover:bg-red-600 focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2"
              >
                Delete
              </button>
            </div>
      ...
      <form onSubmit={formData.id ? handleUpdateText : handleCreateText} className="p-6 mt-8 bg-white rounded-lg shadow-lg">
        ...
        <button type="submit" className="px-4 py-2 font-semibold text-black bg-blue-500 rounded-md shadow-md hover:bg-blue-600 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2">
